...
# CentralAir: NA means that there is no air conditioning
replace_na(df, "CentralAir", value="No")

# EnclosedPorch: NA means that there is no enclosed porch
replace_na(df, "EnclosedPorch", value=0)

# Fireplaces: NA means that there are no fireplaces
replace_na(df, "Fireplaces", value=0)

# SaleCondition: NA means that the the condition of sale is normal
replace_na(df, "SaleCondition", value="Normal")
...
rmse_dl = root_mean_squared_error(np.expm1(y_test), np.expm1(y_pred))

print(f"RMSE: {rmse:.4f}, ({rmse_dl:.2f}$)")
...
def assess_regression_model(
    ...
    # your_code_here
    y_train_trans = np.expm1(y_train)
    y_test_trans = np.expm1(y_test)
    y_pred_train_trans = np.expm1(y_pred_train)
    y_pred_test_trans = np.expm1(y_pred_test)

    # calculate train and test RMSE
    # your_code_here
    rmse_train = root_mean_squared_error(y_train_trans, y_pred_train_trans)
    rmse_test = root_mean_squared_error(y_test_trans, y_pred_test_trans)
...
# LassoCV
reg_lasso = LassoCV(n_alphas=1000, cv=None, random_state=0)
reg_lasso.fit(X_train, y_train)
...
# // skomentuj tutaj
#
# Wynik w przypadku L1 (Lasso) znacznie się poprawił. Test RMSE oraz train RMSE mają o wiele mniejsze wartości, co oznacza, że udało się wyeliminować overfitting. W przypadku L2 różnica między błędami spadła.
...
# drop default, duration, pdays, poutcome columns
df = df.drop(["default", "duration", "pdays", "poutcome"], axis="columns")

# filtering education column
df = df.loc[df["education"] != "illiterate"]

# encoding education, contact, month, day_of_week, y
df = df.replace(
    {
        "month": month_mapping,
...
assert sorted(["primary", "secondary", "tertiary", "unknown"]) == sorted(
    df["education"].unique()
)
...
df = df.astype({"contact": np.int64, "month": np.int64, "day_of_week": np.int64})
...
# %% [markdown]
# // There are no substantial missing values according to the barchart above.
